...
- **렌더된 단어의 총 개수를 단언하지 않는다.** d3-cloud 는 canvas 측정으로 배치하고 안 들어가는 단어를 조용히 버리므로 `toHaveCount(3)` 같은 총량 단언은 폰트 렌더링 차이만으로 깨진다. 개별 단어의 가시성(`toBeVisible()`)과 **특정 단어의 부재**(`toHaveCount(0)`)는 허용되며, 실제로 Task 4·5 가 그렇게 쓴다.
