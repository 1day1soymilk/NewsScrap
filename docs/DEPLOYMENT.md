...
전부 500을 받았기 때문이다. **마이그레이션 `0034`부터는 그 캐시를 읽을 때마다
`keyword_graph_config_fingerprint()`(`scoring_weights` 와 `word_overrides` 를
해시한 값, `note` 류 컬럼은 제외)를 캐시 행의 것과 비교한다.** 값을 바꾸면
해시가 바뀌고, 다음 읽기부터 캐시가 즉시 무효화되어 그 자리에서 다시 계산한
결과를 돌려준다 — 재튜닝이 배포 없이 즉시 반영된다는 문장이 다시 참이다.
캐시 히트 자체는 1.35ms 에서 대략 3~6ms 로 느려지지만(실측, 웜 상태 두 번째
실행), 캐시 미스일 때의 ~2초에 비하면 매번 비교해도 남는 장사다.

`refresh_keyword_graph_cache(date)` 를 손으로 부를 필요는 없어졌지만 함수
자체는 남아 있다 — 재튜닝 직후 그 자리에서 ~2초를 기다리는 대신 특정 날짜의
캐시를 미리 새로 채워 두고 싶을 때 쓴다:

```bash
# 특정 날짜 하나를 미리 채워 두기 (선택 사항)
...
낫는다 (하루 12회, 실행당 여분 1개 — 8일 밀림도 하루 안에 저절로 풀린다). 급하면
`refresh_keyword_graph_cache(date)` 를 손으로 한 번 더 불러도 된다. **마이그레이션
`0034`부터 이 뷰는 `scoring_weights` / `word_overrides` 재튜닝도 본다** — 캐시
행의 `config_fingerprint` 가 현재 값과 다르면 `stale` 을 찍고, 그러면 위
`refresh_stale_keyword_graph_cache` 가 다른 낡음과 똑같이 저절로 새로고침한다.
값을 바꾼 뒤 손으로 할 일이 있다면 그건 오직 위 "미리 채워 두기"뿐이다.
...
잡은 하나가 아니라 **열두 개**이고, 전부 같은 함수를 부른다. 05시부터 23시까지
2시간 간격에 03시 하나, 그리고 자정 직전에 하나:

| jobname | UTC | KST |
| --- | --- | --- |
| `collect-headlines-03kst` | `0 18 * * *` | 03:00 |
| `collect-headlines-05kst` | `0 20 * * *` | 05:00 |
| `collect-headlines-daily` | `0 22 * * *` | 07:00 |
| `collect-headlines-09kst` | `0 0 * * *`  | 09:00 |
| `collect-headlines-11kst` | `0 2 * * *`  | 11:00 |
| `collect-headlines-13kst` | `0 4 * * *`  | 13:00 |
| `collect-headlines-15kst` | `0 6 * * *`  | 15:00 |
| `collect-headlines-17kst` | `0 8 * * *`  | 17:00 |
| `collect-headlines-19kst` | `0 10 * * *` | 19:00 |
| `collect-headlines-21kst` | `0 12 * * *` | 21:00 |
| `collect-headlines-23kst` | `0 14 * * *` | 23:00 |
| `collect-headlines-2350kst` | `50 14 * * *` | 23:50 |

**이름은 낡은 채로 둔다.** `collect-headlines-daily`는 07시에 돌고 `daily`가
아니다. `cron.alter_job`은 이름을 못 바꾸고, 이름을 바꾸려면 평문 service-role
키가 든 명령문을 다시 붙여넣어야 한다. 이 표가 그 대신이다.

**23:50 잡은 미용이 아니라 구멍 하나를 막는다.** 23:00 런 다음이 03:00 런인데
그것은 **다음 날**이라 날짜 경계 정지가 23시~자정 발행분을 off-day로 버린다.
그 시각까지 각 날의 마지막 행이 23:00:07이었다 — 하루 ~100건이 영구히 안
들어오고 있었다. 23:50에 시작한 런은 `todayInSeoul()`을 핸들러 진입에서 읽으므로
자정을 넘겨 끝나도 오늘 날짜로 저장한다.

**이 배포일은 `collect_cap` 150 → 300과 같은 수집 체제 경계다.** 표본이 촘촘해지면
4시간 사이에 300위 밖으로 밀려나던 기사가 잡히므로 하루 총량이 오른다. 두께가
다른 날끼리는 F1을 비교할 수 없다 — `scripts/analysis/README.md`. eval day 다섯
날은 전부 이 앞이다.
...
논리라면 2시간 간격의 크론은 워커를 공유하지 않아야 하지만, **이것은 추론이지
...
분석기가 함수 안에 있으니 외부 한도라는 것이 없고, 열두 번을 돌든 그보다 자주 돌든
