...
호출이 새 워커에서는 200 이고 덥혀진 워커의 세 번째 호출에서는 546 이다. 크론은
4시간 간격이라 워커를 공유하지 않는다. 손으로 연달아 부르는 것이 이것을 밟는
유일한 방법이고, 그때 응답은 **본문이 없다.**

막는 것은 예산이 아니라 **날짜 도장**이다. `collected_date` 는 수집한 날이고 깊은
페이지는 오래된 기사라, 자정을 넘겨 긁으면 어제 뉴스가 오늘로 저장된다. 20:10 KST
실측으로 사회·경제·정치는 441건을 긁어도 오늘을 벗어나지 않지만 (사회는 441건이
3.8시간치다) 생활/문화와 IT 는 290위 언저리에서 08-03 으로 넘어간다. cap 을 300 으로
한 실행 하나가 그런 행 7개를 저장했다.
