...
**cap 은 300 으로 올린다. 300 은 일부러 저 열의 꼭대기가 아니다** — 450 이면 전부
새 기사인 실행이 2,700건이 되는데, 지금까지 측정된 가장 깊은 곳이 2,630 이고 워커
예산은 누적이다. **경계 정지 배포가 먼저**여야 한다: 150 에서도 이미 넘치는데 300
이면 두 배로 넘친다.

올린 뒤 확인할 것은 응답의 **`off-day`** 필드다 — 03시·07시 실행에서는 크고 11시
이후에는 0 에 가까워야 한다. 그리고 위 대조를 다시 돌려 129 가 0 이 되는지 본다.
